...
class UvcCameraService {
  ...
  bool get isRecording => _camera.isRecording;

  int startVideoRecording(String path) => _camera.startVideoRecording(path);

  int stopVideoRecording() => _camera.stopVideoRecording();

  List<UvcCameraControl> supportedControls() {
    try {
      return _camera.supportedControls();
    } catch (_) {
      return [];
    }
  }
  ...
  List<UvcCameraMode> supportedModes() {
    try {
      return _camera.supportedModes();
    } catch (_) {
      return [];
    }
  }

  Future<bool> mudarModo(UvcCameraMode modo) async {
    if (!isConnected) return false;
    try {
      final resultado = await _camera.startPreview(
        modo,
  ...
  int? getControl(UvcControlId id) {
    try {
      return _camera.getControl(id);
    } catch (_) {
      return null;
    }
  }

  bool setControl(UvcControlId id, int value) {
    try {
