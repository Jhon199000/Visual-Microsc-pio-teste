...
/// Ferramenta ativa para desenhar/anotar sobre a imagem da câmera.
enum FerramentaAtiva { nenhuma, desenho, texto }

/// Um texto colocado pelo usuário sobre a imagem, em coordenadas de tela.
...
class _TelaPrincipalState extends State<TelaPrincipal> {
  ...
  // Ferramentas sobre a imagem: desenho à mão livre e texto.
  ...
  // Slider de brilho flutuante sobre a imagem.
  bool _brilhoSliderVisivel = false;
  UvcCameraControl? _controleBrilho;
  double? _valorBrilho;

  // Dividir tela / comparar com uma foto salva.
  ...
  /// Verifica quantos dispositivos UVC estão conectados antes de abrir a
  /// câmera. Se houver mais de um, pergunta ao usuário qual usar.
  ...
  /// Mesma lógica de seleção, usada pelo botão de reconectar.
  ...
  /// Ativa/desativa uma ferramenta (desenho ou texto). Tocar de novo na
  /// ferramenta já ativa a desliga. Ativar uma desliga a outra e sai do
  /// modo de medição, já que só uma ferramenta de toque funciona por vez.
  ...
  /// Apaga todos os desenhos e textos colocados sobre a imagem. Não afeta
  /// medições, grade ou foco central, que têm seus próprios controles.
  ...
  /// Procura, entre os controles UVC disponíveis, o que corresponde ao
  /// brilho da imagem (label contendo "brightness" ou "brilho").
  ...
  /// Liga/desliga o modo "dividir tela": mostra a câmera ao vivo de um
  /// lado e uma foto já salva do outro, para comparação lado a lado.
  ...
  /// Abre a configuração de time-lapse (intervalo entre fotos) e, se
  /// confirmado, inicia a captura periódica.
  ...
  /// Captura a imagem atual e salva uma versão "digitalizada" (preto e
  /// branco, mais contraste), útil para anotações escritas sobre a lâmina.
  ...
  /// Ejeta o microscópio: apenas desconecta, sem tentar reconectar
  /// sozinho (diferente do botão de reconectar, que já existe).
  ...
  /// Envolve widgets de UI (barras de botões) que devem sumir ao tocar na
  /// tela. A coluna lateral esquerda não usa isso: ela fica sempre fixa.
  ...
  /// Tenta identificar o formato (codec) do modo a partir do texto do
  /// `label` retornado pelo plugin UVC (ex.: "MJPEG", "YUYV", "YUY2"...).
  /// Retorna null se não reconhecer nenhum formato conhecido no texto.
  ...
  /// Selo colorido indicando o formato do modo de câmera. MJPEG (comprimido,
  /// costuma liberar resoluções maiores) aparece em verde; formatos brutos
  /// como YUYV/YUY2 aparecem em âmbar.
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // Traços de desenho à mão livre e textos, sempre visíveis mesmo
          // quando a ferramenta não está ativa (para continuar vendo o que
          // já foi desenhado enquanto navega pela imagem).
          ...
          // Coluna esquerda: câmera, vídeo, régua, zoom, atualizar, ejetar
          Positioned(
            left: 16,
            top: 0,
            bottom: 0,
          ...
          // Coluna direita: galeria, luz, cores, configurações
          Positioned(
            right: 16,
            top: 16,
            bottom: 16,
          ...
          // Barra secundária: girar, espelhar, identificar com IA
          Positioned(
            top: 16,
            left: 0,
            right: 0,
          ...
          // Barra flutuante de funções adicionais: dividir tela, time-lapse
          // e digitalizar.
          Positioned(
            bottom: 76,
            left: 0,
          ...
          // Barra flutuante de ferramentas sobre a imagem: texto, desenho,
          // apagar e brilho.
          Positioned(
            bottom: 24,
            left: 0,
  ...
  Widget _buildPreview() {
    ...
    if (_controller.estaConectado && _controller.textureId != null) {
      ...
      // Modo dividir tela: ao vivo à esquerda, foto salva à direita.
      return Row(
        children: [
...
/// Desenha os traços à mão livre e os textos colocados pelo usuário sobre
/// a imagem da câmera.
class _DesenhoPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _DesenhoPainter oldDelegate) {
    // As listas são mutadas no lugar (add/clear), então comparar por
    // referência com "!=" nunca detectaria mudança. Sempre redesenha; o
    // custo é baixo e evita que traços/textos novos deixem de aparecer.
    return true;
  }
}

/// Desenha uma grade 3x3 (regra dos terços) sobre a imagem da câmera.
...
/// Desenha uma mira de foco no centro da tela: círculo com cruz e cantos,
/// como referência visual para centralizar a amostra.
class _FocoCentralPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Cantos, como uma mira de foco de câmera
